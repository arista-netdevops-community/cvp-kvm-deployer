#!/bin/bash
# ===========================================================================================
# == Deployment of CloudVision KVM VM (Noris Network AG - Offline Build)
# == DO NOT SHARE WITH OTHER CUSTOMERS!
# == Specs can be customized below
# == RHEL/CentOS 8 has been used as a hypervisor OS
# ===========================================================================================
# == Florian Hibler <florian@arista.com>
# ===========================================================================================

PWD=$(pwd)
CENTOS_ISO=/root/CentOS-7-x86_64-Minimal-1908.iso
CVP_INSTALLER=/root/cvp-rpm-installer-2020.3.0
VM_VCPU=8
VM_MEM=17408
VM_DISKSIZE_ROOT=35
...
function copy_centos {
    ...
    cp $CENTOS_ISO /var/lib/libvirt/boot/CentOS-7-x86_64-Minimal-1908.iso
}

function generate_cloudvision_image {
    genisoimage -allow-limited-size -l -J -r -iso-level 3 -o /var/lib/libvirt/images/cvp.iso $CVP_INSTALLER
}
...
function install {
    ...
    virt-xml cvp --remove-device --disk 3
    # attach-disk --domain cvp --source /var/lib/libvirt/images/cvp.iso --targetbus sata --type cdrom --target sda --config
